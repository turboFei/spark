...
/**
  ...
  */
private[spark]
final class ShuffleSplitBlockFetcherIterator(
    context: TaskContext,
  ...
  import ShuffleSplitBlockFetcherIterator._
  ...
  /**
    ...
    */
  private[this] val results = new LinkedBlockingQueue[SplitFetchResult]

  /**
    * Current [[SplitFetchResult]] being processed. We track this so we can release the current buffer
    * in case of a runtime exception when processing the current buffer.
    */
  @volatile private[this] var currentResult: SuccessSplitFetchResult = null

  /**
    * Queue of fetch requests to issue; we'll pull requests off this gradually to make sure that
    * the number of bytes in flight is limited to maxBytesInFlight.
    */
  private[this] val fetchRequests = new Queue[SplitFetchRequest]
  ...
  /**
    ...
    */
  private[this] val deferredFetchRequests = new HashMap[BlockManagerId, Queue[SplitFetchRequest]]()
  ...
  private[this] def sendRequest(req: SplitFetchRequest) {
    ...
    val blockFetchingListener = new SplitBlockFetchingListener {
      override def onBlockFetchSuccess(blockId: String, splitId: Int, buf: ManagedBuffer): Unit = {
        // Only add the buffer to results queue if the iterator is not zombie,
        // i.e. cleanup() has not been called yet.
        ShuffleSplitBlockFetcherIterator.this.synchronized {
          if (!isZombie) {
      ...
      override def onBlockFetchFailure(blockId: String, splitId: Int, e: Throwable): Unit = {
        logError(s"Failed to get block(s) from ${req.address.host}:${req.address.port}", e)
        results.put(new FailureFetchResult(BlockId(blockId), splitId, address, e))
      }
    }
    ...
    if (req.size > maxReqSizeShuffleToMem) {
      shuffleClient.fetchSplitBlocks(address.host, address.port, address.executorId, blockIds.toArray,
  ...
  private[this] def splitLocalRemoteBlocks(): ArrayBuffer[SplitFetchRequest] = {
    ...
    val remoteRequests = new ArrayBuffer[SplitFetchRequest]
  ...
  override def next(): (BlockId, InputStream) = {
    ...
    var result: SplitFetchResult = null
    ...
    currentResult = result.asInstanceOf[SuccessSplitFetchResult]
    (currentResult.blockId, new SplitBufferReleasingInputStream(input, this))
  }
  ...
  private def fetchUpToMaxBytes(): Unit = {
    ...
    while (isRemoteBlockFetchable(fetchRequests)) {
      ...
      if (isRemoteAddressMaxedOut(remoteAddress, request)) {
        ...
        val defReqQueue = deferredFetchRequests.getOrElse(remoteAddress, new Queue[SplitFetchRequest]())
    ...
    def send(remoteAddress: BlockManagerId, request: SplitFetchRequest): Unit = {
      sendRequest(request)
      numBlocksInFlightPerAddress(remoteAddress) =
        numBlocksInFlightPerAddress.getOrElse(remoteAddress, 0) + request.blocks.size
    }

    def isRemoteBlockFetchable(fetchReqQueue: Queue[SplitFetchRequest]): Boolean = {
    ...
    def isRemoteAddressMaxedOut(remoteAddress: BlockManagerId, request: SplitFetchRequest): Boolean = {
      numBlocksInFlightPerAddress.getOrElse(remoteAddress, 0) + request.blocks.size >
...
/**
  ...
  */
private class SplitBufferReleasingInputStream(
    private val delegate: InputStream,
    private val iterator: ShuffleSplitBlockFetcherIterator)
  extends InputStream {
...
private[storage]
object ShuffleSplitBlockFetcherIterator {
  ...
  private[storage] case class SuccessSplitFetchResult(
      ...
      size: Long,
      buf: ManagedBuffer,
      isNetworkReqDone: Boolean) extends SplitFetchResult {
    require(buf != null)
    require(size >= 0)
  }
