 ...
 * be reopened again.
 */
private[spark] class SplitDiskBlockObjectWriter(
    val file: File,
  ...
  def recordWritten(): Unit = {
    ...
    if (numRecordsWritten % 16384 == 0) {
      updateBytesWritten()
      val n = numRecordsWritten / 16384
      val pos = channel.position()
      if (pos -committedPosition >= splitThreshold * ( n +1)/(n +2)) {
        val fileSegment = commitAndGet()
        splitLengths += fileSegment.length
      }
    }
  }
  ...
  // for the invoke in java code
  def getSplitLengthsArray(): Array[Long] = splitLengths.toArray
