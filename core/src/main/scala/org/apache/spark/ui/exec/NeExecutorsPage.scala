...
private[ui] class NeExecutorsPage(parent: NeExecutorsTab, threadDumpEnabled: Boolean)
  extends WebUIPage("") {
  ...
  def render(request: HttpServletRequest): Seq[Node] = {
    val (activeExecutorInfo, deadExecutorInfo) = listener.synchronized {
      ...
      val _activeExecutorInfo = {
        for (statusId <- listener.activeStorageStatusList.indices)
          yield ExecutorsPage.getExecInfo(listener, statusId, isActive = true)
      }
      val _deadExecutorInfo = {
        for (statusId <- listener.deadStorageStatusList.indices)
    ...
    implicit val idOrder: Ordering[String] = Ordering[Int].on((s: String) => idStrToInt(s)).reverse
    val execInfoSorted = execInfo.sortBy(_.id)
    val logsExist = execInfo.exists(_.executorLogs.nonEmpty)
