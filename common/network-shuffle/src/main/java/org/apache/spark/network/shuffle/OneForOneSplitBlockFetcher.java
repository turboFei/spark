...
import org.apache.spark.network.shuffle.protocol.OpenSplitBlocks;
 ...
 */
public class OneForOneSplitBlockFetcher {
  private static final Logger logger = LoggerFactory.getLogger(OneForOneSplitBlockFetcher.class);

  private final TransportClient client;
  private final OpenSplitBlocks openMessage;
  ...
  public OneForOneSplitBlockFetcher(
  ...
  public OneForOneSplitBlockFetcher(
    ...
    this.client = client;
    this.openMessage = new OpenSplitBlocks(appId, execId, blockIds);
  ...
  private class ChunkCallback implements ChunkReceivedCallback {
    ...
    public void onSuccess(int chunkIndex, ManagedBuffer buffer) {
      ...
      //TODO: splitId
      String  blockId =  blockIds[chunkIndex].substring(0, blockIds[chunkIndex].lastIndexOf('_'));
      int splitId = Integer.parseInt(blockIds[chunkIndex].substring(blockIds[chunkIndex].lastIndexOf('_')+1 , blockIds[chunkIndex].length()));
      listener.onBlockFetchSuccess(blockId, splitId, buffer);
    }
  ...
  private void failRemainingBlocks(String[] failedBlockIds, Throwable e) {
    for (String blockId : failedBlockIds) {
      try {
        ...
        int spId = Integer.parseInt(blockId.substring(blockId.lastIndexOf('_') +1, blockId.length()));
        listener.onBlockFetchFailure(blockId, spId, e);
      } catch (Exception e2) {
  ...
  private class DownloadCallback implements StreamCallback {
    ...
    public void onComplete(String streamId) throws IOException {
      String  blockId =  blockIds[chunkIndex].substring(0, blockIds[chunkIndex].lastIndexOf('_'));
      int splitId = Integer.parseInt(blockIds[chunkIndex].substring(blockIds[chunkIndex].lastIndexOf('_')+1 , blockIds[chunkIndex].length()));
      listener.onBlockFetchSuccess(blockId, splitId, channel.closeAndRead());
